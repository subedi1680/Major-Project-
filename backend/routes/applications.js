...
const { auth, requireUserType, requireAnyUserType } = require('../middleware/auth');
...
router.post('/', [
    ...
    body('expectedSalary.amount').optional().isNumeric().withMessage('Expected salary must be a number'),
    body('expectedSalary.period').optional().isIn(['hourly', 'monthly', 'yearly']).withMessage('Invalid salary period'),
    ...
    try {
        ...
        const { jobId, coverLetter, expectedSalary, availableStartDate, questionsAnswers } = req.body;
...
// @access  Private (Job seekers only)
router.get('/my-applications', [auth, requireUserType('jobseeker')], async (req, res) => {
    try {
        ...
        const applications = await Application.getByApplicant(req.user.userId, filters)
            .skip((page - 1) * limit)
...
router.get('/employer/received', [auth, requireUserType('employer')], async (req, res) => {
    try {
        ...
        const applications = await Application.getByEmployer(req.user.userId, filters)
            .skip((page - 1) * limit)
            .limit(parseInt(limit));
...
router.get('/:id', [auth, requireAnyUserType(['jobseeker', 'employer'])], async (req, res) => {
    try {
        const application = await Application.findById(req.params.id)
            .populate('job', 'title companyName location jobType salary requirements responsibilities')
        ...
        const isEmployer = application.employer._id.toString() === req.user.userId;
...
router.put('/:id/status', [
    ...
    body('status').isIn(['pending', 'reviewed', 'shortlisted', 'interview-scheduled', 'interview-completed', 'offered', 'hired', 'rejected']).withMessage('Invalid status'),
    body('note').optional().isLength({ max: 500 }).withMessage('Note cannot exceed 500 characters'),
    body('rejectionReason').optional().isIn(['qualifications', 'experience', 'skills', 'salary-expectations', 'location', 'availability', 'cultural-fit', 'position-filled', 'other']),
    body('rejectionFeedback').optional().isLength({ max: 500 }).withMessage('Rejection feedback cannot exceed 500 characters')
], async (req, res) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
...
router.post('/:id/notes', [
    auth,
    requireUserType('employer'),
    body('content').trim().isLength({ min: 1, max: 1000 }).withMessage('Note content must be between 1 and 1000 characters'),
...
router.put('/:id/withdraw', [auth, requireUserType('jobseeker')], async (req, res) => {
    try {
        const application = await Application.findById(req.params.id);
        ...
        await application.updateStatus('withdrawn', req.user.userId, 'Application withdrawn by candidate');
...
// @access  Private (Employers only)
router.get('/stats/employer', [auth, requireUserType('employer')], async (req, res) => {
    try {
