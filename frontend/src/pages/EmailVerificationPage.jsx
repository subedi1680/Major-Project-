...
function EmailVerificationPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      // Use AuthContext's verifyEmail method
      const result = await verifyEmail({ email, pin: pinCode });

      if (result.success) {
        setSuccess(true);
        // Account created successfully, redirect to appropriate dashboard after delay
        setTimeout(() => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-dark-950 via-dark-900 to-dark-800 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-md w-full space-y-8 relative animate-fade-in">
        ...
        <div className="glass-card p-4 rounded-xl text-center">
          ...
          <p
            className={`text-2xl font-bold ${timeLeft < 60 ? "text-red-400" : "text-primary-400"}`}
          >
            {formatTime(timeLeft)}
          </p>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-slate-300">
              ...
              <p className="text-slate-400">
                The PIN may take a few moments to arrive. Check your spam folder
                if you don't see it.
              </p>
            </div>
          </div>
